...
import { Check, Edit, Loader2, X, Users, ChevronLeft, ChevronRight, Plus, Trash2 } from "lucide-react";
...
export default function TeamPage() {
  ...
  const { mutate: deleteTeam, status: deleteStatus } = useMutation({
    ...
    onSuccess: (_, teamId) => {
      toast({ title: "Team deleted", description: "Team has been removed.", duration: 3000 });
      queryClient.invalidateQueries({ queryKey: ["employee_teams"] });
      setSelectedId(null);
    },
    onError: () => {
            ...
            <tbody className="divide-y divide-border/40">
              {isLoading ? (
                Array.from({ length: 5 }).map((_, idx) => (
                  <tr key={idx}>
                    <td colSpan={4} className="h-14 animate-pulse bg-muted/20" />
                  </tr>
                ))
              ) : teams.length > 0 ? (
                teams.map((team) => (
