...
const Page = () => {
  ...
  const verifyOtpMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "OTP Validation Successful", description: "Redirecting to Login Page" });
  ...
  const resendOtpMutation = useMutation({
    mutationFn: async () => {
      const response = await axios.post(`${backendUrl}/otp/resend`, {
        email: storedemail,
        reason: "verify_user",
      });
      return response.data;
              ...
              {slots.map((i) => (
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setValue(newVal);
                    if (char && i < 5) {
                      const next = document.getElementById(`verify-otp-slot-${i + 1}`);
                      next?.focus();
                    }
                  }}
                  onKeyDown={(e) => {
                    if (e.key === "Backspace" && !value[i] && i > 0) {
                      const prev = document.getElementById(`verify-otp-slot-${i - 1}`);
                      prev?.focus();
                    }
                  }}
                  id={`verify-otp-slot-${i}`}
