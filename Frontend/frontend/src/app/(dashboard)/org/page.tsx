...
function buildGraph(data: OrgStructure): { nodes: Node[]; edges: Edge[] } {
  ...
  const { admins = [], departments = [] } = data;
  const USER_SLOT = NODE_W.user + 24; // width + gap between leaf nodes

  // Pass 1: assign each user node a unique X slot using a global cursor
  let cursor = 0;

  type TeamInfo = { teamNodeId: string; userXs: number[]; cx: number };
  type DeptInfo = { deptNodeId: string; teams: TeamInfo[]; cx: number };

  const deptInfos: DeptInfo[] = departments.map((dept) => {
    const deptNodeId = `dept-${dept.id}`;
    const teamInfos: TeamInfo[] = dept.teams.map((team) => {
      const teamNodeId = `team-${team.id}`;
      const userXs: number[] = [];

      if (team.users.length === 0) {
        // allocate one slot so the team node has width to sit above
        userXs.push(cursor);
        cursor += USER_SLOT;
      } else {
        team.users.forEach(() => {
      ...
      const cx = (userXs[0] + userXs[userXs.length - 1]) / 2;
      return { teamNodeId, userXs, cx };
    });

    // dept centre = average of its teams' centres (or a single slot if no teams)
    let cx: number;
    if (teamInfos.length === 0) {
      cx = cursor;
      cursor += USER_SLOT;
    } else {
      cx = (teamInfos[0].cx + teamInfos[teamInfos.length - 1].cx) / 2;
    }

    return { deptNodeId, teams: teamInfos, cx };
  });

  // Centre everything around x = 0
  const totalWidth = cursor - USER_SLOT;
  const shift = totalWidth / 2;

  // Admin nodes: centred above all departments
  const adminCX =
    deptInfos.length > 0
      ? (deptInfos[0].cx + deptInfos[deptInfos.length - 1].cx) / 2 - shift
      : 0;

  admins.forEach((admin, i) => {
    const x = adminCX + (i - (admins.length - 1) / 2) * GAP.adminX;
    nodes.push({
      id: `admin-${admin.id}`,
      type: "adminNode",
  ...
  // Dept, Team and User nodes
  departments.forEach((dept, di) => {
    const { deptNodeId, teams: teamInfos, cx: deptCX } = deptInfos[di];
    const dx = deptCX - shift;
    const totalUserCount =
      dept.teams.reduce((s, t) => s + t.users.length, 0) + dept.directUsers.length;

    nodes.push({
      id: deptNodeId,
      type: "deptNode",
      position: { x: dx, y: TIER_Y.dept },
      data: { name: dept.name, lead: dept.lead, teamCount: dept.teams.length, userCount: totalUserCount },
    ...
    teamInfos.forEach(({ teamNodeId, userXs, cx: teamCX }) => {
      const team = dept.teams.find((t) => `team-${t.id}` === teamNodeId)!;
      const tx = teamCX - shift;

      nodes.push({
        id: teamNodeId,
        type: "teamNode",
        position: { x: tx, y: TIER_Y.team },
      ...
      team.users.forEach((user, ui) => {
        ...
        const ux = userXs[ui] - shift;

        nodes.push({
          id: userNodeId,
          type: "userNode",
          position: { x: ux, y: TIER_Y.user },
          data: { ...user },
